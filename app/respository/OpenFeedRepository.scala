...
import domain.{CustomFeed, OpenFeed, Stats}
 ...
 */
class OpenFeedRepository extends CassandraTable[OpenFeedRepository, OpenFeed] {
  ...
  override def fromRow(row: Row): OpenFeed = {
    OpenFeed(
      zone(row),
      siteCode(row),
      id(row),
...
object OpenFeedRepository extends OpenFeedRepository with DataConnection {
  override lazy val tableName = "ofeeds"

  def save(feed: OpenFeed): Future[ResultSet] = {
    insert
  ...
  def getFeedById(zone: String, siteCode: String, id: String): Future[Option[OpenFeed]] = {
    select.where(_.zone eqs zone).and(_.siteCode eqs siteCode).and(_.id eqs id).one()
  }

  def getFeedsBySite(zone: String, siteCode: String): Future[Seq[OpenFeed]] = {
    select.where(_.zone eqs zone).and(_.siteCode eqs siteCode).fetchEnumerator() run Iteratee.collect()
  }

  def getFeedsByZone(zone: String): Future[Seq[OpenFeed]] = {
    select.where(_.zone eqs zone).fetchEnumerator() run Iteratee.collect()
  }

  def getFeeds: Future[Seq[OpenFeed]] = {
    select.fetchEnumerator() run Iteratee.collect()
  }

  def deleteFeed(zone: String, sitecode: String, id: String): Future[ResultSet] = {
    delete.where(_.zone eqs zone).and(_.siteCode eqs sitecode).and(_.id eqs id).future();
